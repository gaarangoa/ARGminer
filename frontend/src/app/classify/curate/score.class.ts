...
export class ScoreAnnotation {
    ...
    score(ARG: object, VAL: object){
        ...
        let gclass = this.getAllSubstrings(JSON.stringify([VAL['class']]).replace(/\W|\_/g,"").toUpperCase(),3).filter(function(item, i, ar){ return ar.indexOf(item) === i; });

        let ggroup = this.getAllSubstrings(JSON.stringify([VAL['group']]).replace(/\W|\_/g,"").toUpperCase(), 3).filter(function(item, i, ar){ return ar.indexOf(item) === i; });

        let gmech = this.getAllSubstrings(JSON.stringify([VAL['mechanism']]).replace(/\W|\_/g,"").toUpperCase(), 3).filter(function(item, i, ar){ return ar.indexOf(item) === i; });
